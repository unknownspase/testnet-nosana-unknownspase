...
domanlist = ["1secmail.com", "1secmail.org", "1secmail.net", "xojxe.com", "wwjmp.com", "esiix.com", "vddaz.com"]
random_domain = random.choice(domanlist)
dm = random_domain
...
def mainth():
	while True:
		try:
			...
			body = {"password":str(password)+"!","email":str(username)+"@"+(dm),"name":str(nameOfAcc),"udid":udid,"referralCode":str(refferalCode)}
			...
			if r.status_code == 200:
				...
				for i in range(500):
					r = get(f"https://www.1secmail.com/api/v1/?action=getMessages&login={username}&domain={dm}")
					r_json = r.json()
					if len(r_json) > 0:
						latest_mail = r_json[0].get('id')
						req = get(f"https://www.1secmail.com/mailbox/?action=mailBody&id={latest_mail}&login={username}&domain={dm}")
						# time.sleep(10)
			...
			with open('LegionAccounts.txt', 'a', encoding='utf-8') as file:
				file.write(f'{str(password)}!:{str(username)}@{dm}:{str(nameOfAcc)}:{udid}\n')
